...
#计算百分比
df['合格%']=df.apply(lambda x:x.合格/x.合计*100,axis=1)
df['差大%']=df.apply(lambda x:x.差大/x.合计*100,axis=1)
df['有差%']=df.apply(lambda x:x.有差/x.合计*100,axis=1)
df['相等%']=df.apply(lambda x:x.相等/x.合计*100,axis=1)
df.sort_values(by=['合格%'],ascending=True)
